...
ARG APP_VERSION_DISPLAY=VERSION
...
COPY key.properties /tmp/build/amphi-notes/android/key.properties
...
RUN mkdir -p /tmp/build/amphi-notes/result

WORKDIR /tmp/build/amphi-notes

RUN mv ./build/app/outputs/flutter-apk/app-release.apk ./result/Notes-${APP_VERSION_DISPLAY}-Android.apk \
&& mv ./build/app/outputs/flutter-apk/app-armeabi-v7a-release.apk ./result/Notes-${APP_VERSION_DISPLAY}-Android-armeabi_v7a.apk \
&& mv ./build/app/outputs/flutter-apk/app-x86_64-release.apk ./result/Notes-${APP_VERSION_DISPLAY}-Android-x86_64.apk \
&& mv ./build/app/outputs/flutter-apk/app-arm64-v8a-release.apk ./result/Notes-${APP_VERSION_DISPLAY}-Android-arm64_v8a.apk

# docker build -f build-apk.dockerfile -t amphi-notes-apk-builder .
# docker create --name build-output-apk amphi-notes-apk-builder
# docker cp build-output-apk:/tmp/build/amphi-notes/result/. ./result